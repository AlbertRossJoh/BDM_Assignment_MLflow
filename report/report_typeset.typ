...
The main discovery process has been in a python notebook, with models and model metrics being logged to `mlflow`. The process included building a meta-framework for automating experiments, and for building "immutable" pipelines#footnote([The builder returns a new pipeline on every call rather than mutating in place.]). The main purpose of this meta-framework was to (1) make pipeline updates simple and immutable and (2) making experiments easy and exhaustive. As it can be hard to figure out what's the best combination of transformers/regressors, the main method employed in this project is doing exhaustive search on pipelines. Exhaustive search in this instance means, building a pipeline with different transformers, and trying all valid#footnote([Some pipeline steps are not compatible, e.g. providing both direction encoded as sin/cos and using the `OneHotEncoder` does not make much sense]) combinations. Each of these combinations is then tried on multiple regressors, thus we exhaustively try all possible combinations of transformers and regressors. Each of these runs is recorded to mlflow, where we can view the properties of individual runs. For an example of the exhaustive pipelines see #link(<appendix:exhaustive>)[Appendix A].
...
As we see, this down-sampling causes an increase in the 30 MW range and in the 15 MW range. This is most likely some large values affecting the mean #footnote([This can be corrected for using the median, however this minimally worsens the $R^2$ score of the models.]).
...
The raw datasets contain no null values. Nulls are only introduced in the join and resampling step described in @sec:alignment, these are resolved at resampling time, such that they're not propogated. The null handling is done in the `Speed` column by linear interpolation, and `Direction` is handled by forward filling. Thus no imputation step is needed to fill nulls for input data values. There exists a rolling mean step in the pipeline, which handles missing values at the start and at the end of the dataframe by using the mean.
...
i.e if we map the classes to integers $i in {0, 1, ..., 15}$, then we compute the wind direction in the following manner $theta_i = i dot 22.5$. This also preserves the circular dependency which is discarded by the `OneHotEncoder`.
...
An interesting observation is that the different models seem to perform worse on the same fold, specifically fold 2 seems to contain some data which lowers the floor considerably. If this run is removed then the mean $R^2$ jumps to $0.72$ for the best model, which remains the same, that is 6 _pp_! This is probably explained by the fact that there are a lot of missing observations in the power dataset.
#figure(
...
The correlation matrix shows that there is a small negative correlation between the ordinal day (doy) and the total power output, and the same correlation for speed. Which could imply some hidden signal.
