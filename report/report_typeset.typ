...
With some models it can make sense to scale features as larger features might skew the loss function, causing other features to be undermined. To do this a `StandardScaler` was applied. A `MinMaxScaler` was also tried. The main difference between the two is that the `StandardScaler` and the `MinMaxScaler` is that the standard scaler removes the mean such that the dataset has unit variance @standardscaler (i.e. the z-score is calculated) $z=(x-mu)/sigma$. The `MinMaxScaler` scales each feature such that it is in the range between 0 and 1 @minmaxscaler. The best model ended up including neither scalers, but they're explained in this section for the sake of completeness and transparency. Another important note is that for boosted trees, scalers does not make any performance difference. The reason is that boosted trees makes a series of "splits", meaning that monotonic transformations (such as scaling) have no real impact.
