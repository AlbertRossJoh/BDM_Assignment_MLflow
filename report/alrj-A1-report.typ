...
Most of the libraries are for convenience, however, I should specifically highlight the use of `sklearn` @scikit-learn, a very common ML library, and the usage of `mlflow` @mlflow, a tool for comparing and versioning models.

The main discovery process has been in a python notebook, with models and model metrics being logged to `mlflow`. The process included building a meta-framework for automating experiments, and for building "immutable" pipelines#footnote([The builder returns a new pipeline on every call rather than mutating in place.]). The main purpose of this meta-framework was to (1) make pipeline updates simple and immutable and (2) making experiments easy and exhaustive. As it can be hard to figure out what's the best combination of transformers/regressors, the main method employed in this project is doing exhaustive search on pipelines. Exhaustive search in this instance means, building a pipeline with different transformers, and trying all valid#footnote([Some pipeline steps are not compatible, e.g. providing both direction encoded as sin/cos and using the `OneHotEncoder` does not make much sense]) combinations. Each of these combinations is then tried on multiple regressors, thus I exhaustively try all possible combinations of transformers and regressors. Each of these runs is recorded to mlflow, where I can view the properties of individual runs. For an example of the exhaustive pipelines see #link(<appendix:exhaustive>)[Appendix A].
...
As explained in @sec:metrics, the main metric used is mean $R^2$. To find the model with the best mean $R^2$ I just sort by descending for that column in the run table. I can also choose to view the data as a histogram, however, because of the exhaustive runs, this was impractical.
...
The best model was tested against `future.csv`. For this the model was refitted using a single unshuffled train/test split rather than CV, since the folds were only needed for model comparison; for the final forecast I instead want one model fitted on the largest possible contiguous training window. The output was appended (#text([red line], fill: red)) to the existing dataset(#text([blue line], fill: blue)) and plotted, see @future:forecast.
