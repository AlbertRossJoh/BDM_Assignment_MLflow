...
Another observation is that the mean value of the first fold is really bad; however, the median seems unaffected. This is caused by 4 abysmal linear regression runs, which are thus not representative of the overall performance on fold 0. If these are dropped, the mean of fold 0 jumps to 0.550, which seems more representative.
